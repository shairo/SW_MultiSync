...
rem   dist\SW_MultiSync_v<version>\   (staging folder = contents of the zip)
rem   dist\SW_MultiSync_v<version>.zip
...
set NAME=SW_MultiSync_v%VER%
set STAGE=dist\%NAME%
if exist "%STAGE%" rmdir /s /q "%STAGE%"
mkdir "%STAGE%"

copy /y build\SWMultiSync.exe "%STAGE%\" >nul
