...
| 0x9D 157 | 67 | **EXPLOSION** ✅: `u32 kind (14) · u32 1 · double[3] world pos · f32[3] (0,1,0) up · f32[3] 0 · f32 size · u8 1 · u16 0xFFFF`. `size` = 1.0 / 2.0 / 4.0 for the small / medium / large bomb part (`session_20260919_135657_754`: 3 × each with vehicle damage OFF then ON) and 1.0 for the missile warhead (`_134315_888` veh 76, `_134846_721` veh 81). Emitted at the TRUE position, in the same message as the vehicle's 0x81 — the missile's carried ETA +29, so clients draw the blast ~29 ticks (≈160 m at 340 m/s) ahead of where they show the missile. Bracketed by 0x1A (detonate button press/release) for bomb parts. Damage OFF: nothing else. Damage ON: 0x35 body damage next tick (magnitude 5000 / 10000 / 20000 with value2 2.5 / 4.0 / 4.0 for the primary body, 1200 / 0 for secondaries, other vehicles in range too — veh 11 on the large one), and the large bomb's hit despawned its own vehicle (0x2C+0x38 in the same message). The missile kept flying after its explosion in both runs |
